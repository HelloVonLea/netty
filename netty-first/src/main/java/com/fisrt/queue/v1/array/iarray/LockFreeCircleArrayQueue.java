package com.fisrt.queue.v1.array.iarray;
 ...
 */
public class LockFreeCircleArrayQueue<E> implements Queue<E> {

    protected static final long P_INDEX_OFFSET = UnsafeUtil.fieldOffset(LockFreeCircleArrayQueue.class, "producerIndex");
    protected final static long C_INDEX_OFFSET = UnsafeUtil.fieldOffset(LockFreeCircleArrayQueue.class, "consumerIndex");
    ...
    private AtomicLong prodIndex = new AtomicLong(1);

    public LockFreeCircleArrayQueue(int capacity) {
        //保证容量比2大
...
//                    String obj = queue.dequeue();
    ...
    private static void test3(int n) throws InterruptedException {
        Queue<Integer> queue = new LockFreeCircleArrayQueue<>(4);
        //子线程结束后，main线程再结束控制器
...
//        System.out.println(Thread.currentThread().getName()+"\tcIndex="+cIndex+"\t seq="+seq+"\t消费元素["+e+"]后sequenceBuffer是："+Arrays.toString(sBuffer));
    ...
    private static class UnsafeLongArrayAccess {
        //long[]的offset
